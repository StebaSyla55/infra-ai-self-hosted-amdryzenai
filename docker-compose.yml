...
services:
  ...
  db:
    ...
    healthcheck:
      test: ["CMD-SHELL","pg_isready -U supabase_admin -h localhost -p ${POSTGRES_PORT}"]
      interval: 5s
      timeout: 5s
      retries: 20
    volumes:
  ...
  db-init:
    ...
    command: >
      "
      psql -h db -U supabase_admin -d ${POSTGRES_DB} -p ${POSTGRES_PORT}
      -c \"DO $$
      BEGIN
  ...
  n8n-api:
    ...
    entrypoint: >
      /bin/sh -c "
        n8n import:credentials --separate --input ${N8N_IMPORT_EXPORT_DIR} || true &&
        n8n import:workflow    --separate --input ${N8N_IMPORT_EXPORT_DIR} || true &&
        n8n start
      "
    environment:
      <<: *common-env
  ...
  pg-backup:
    ...
    environment:
      PGPASSWORD: ${POSTGRES_PASSWORD}
      DB_URI: postgresql://${N8N_DB_USER}:${N8N_DB_PASSWORD}@db:${POSTGRES_PORT}/n8n
    volumes:
